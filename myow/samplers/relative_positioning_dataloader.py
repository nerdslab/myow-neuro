...
class RandomIndexSampler(Sampler):
    ...
    def __init__(self, num_nodes: int, batch_size: int, shuffle: bool = False, drop_last: bool = True):
        super(RandomIndexSampler, self).__init__(None)
...
class RelativePositioningDataLoader(DataLoader):
    r"""DataLoader that generate relative positioning pair of views."""
    ...
    def __collate__(self, node_idx):
        node_idx = node_idx[0]  # sampled nodes (batch_size)
        ...
        for key, item in self.data:
            if isinstance(item, Tensor) and item.size(0) == self.num_nodes:
                data[key] = item[cn_mask].clone()

        # generate view 2
        view_2_index = data.pos_edge_index[1]
        view_2 = self.transform(data.x.clone(), data.batch) if self.transform is not None else data.x.clone()
        view_2 = view_2[view_2_index]

        # generate main view (view 1 and m)
        ...
        ret = {
            'data': data.x,
            'view_1_index': view_1_index,
            'view_2': view_2,
            'ccand_edge_index': data.ccand_edge_index,
        }
        return ret
